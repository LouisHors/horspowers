...
**Horspower** is a Chinese-enhanced fork of [obra/superpowers](https://github.com/obra/superpowers) - a skills library for Codex that provides composable workflows for software development. It's a plugin that injects skills at session start and provides slash commands for common workflows.

**Key concept:** Skills are Markdown files with YAML frontmatter that Codex invokes via the `Skill` tool. The `using-horspowers` skill (originally called `using-superpowers` in the upstream project) is injected automatically at session start via a hook.
...
./tests/Codex/run-skill-tests.sh --test test-subagent-driven-development.sh --verbose
```
...
- **Description trap:** Never include workflow steps in description - Codex will follow description instead of reading the skill
...
The `disable-model-invocation: true` prevents Codex from invoking commands - only users can invoke slash commands.
...
- Personal skills (`~/.Codex/skills/`) override horspowers skills
...
Skills are tested using headless Codex sessions:

```bash
Codex -p "prompt" --allowed-tools=all --add-dir "$TEST_DIR" --permission-mode bypassPermissions
```
...
- **Codex:** Native plugin via marketplace (this repo)
- **Codex:** See `docs/README.codex.md` - uses `lib/skills-core.js`
- **OpenCode:** See `docs/README.opencode.md` - shares `lib/skills-core.js`

## Common Pitfalls

**The Description Trap:** If description summarizes the workflow, Codex follows description instead of reading the skill. Keep descriptions trigger-only: "Use when X" not "Use when X to do Y via Z".
...
This repository supports Codex through GitHub Actions.
...
These files are the source of truth for allowed tasks, restricted changes, verification, and PR requirements.
