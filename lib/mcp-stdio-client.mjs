...
class McpStdioSession {
  ...
  #onStdout(chunk) {
    ...
    while (true) {
      ...
      if (newline === -1) {
        if (this.#stdoutBuffer.length > this.#maxResponseBytes) this.fail('mcp_response_too_large');
        return;
      }
      ...
      if (lineBytes.length > this.#maxResponseBytes) {
...
export class McpStdioClient {
  ...
  async run(callback) {
    ...
    try {
      return await callback(connection.session);
    } catch {
