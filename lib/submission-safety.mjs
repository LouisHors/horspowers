...
function hasHighEntropyCredential(value, ignoredTokens) {
  for (const match of value.matchAll(/[A-Za-z0-9+/_=-]{20,256}/gu)) {
    ...
    if (entropy(candidate) >= 3.5) return true;
  }
  return false;
}
...
function inspectSubmissionTextWithOptions(value, options = {}) {
  ...
  if (hasHighEntropyCredential(value, options?.ignoredHighEntropyTokens)) {
...
function referenceLogicalIds(document) {
  return new Set(document.references.map(reference => reference.logical_id));
}
...
export async function validateAndSerializeSafeDocument(content, projectRoot, {
  ...
  const inspected = inspectSubmissionTextWithOptions(markdown, {
    serialized: true,
    ignoredHighEntropyTokens: referenceLogicalIds(validated.document)
  });
