...
function exactObject(value, keys, pathname, errors) {
  ...
  for (const key of Object.keys(value)) {
    if (!keys.includes(key)) errors.push({ path: `${pathname}.unknown_field`, code: 'unknown_field' });
  }
...
function hasCredentialCommandArgument(value) {
  const normalized = typeof value === 'string' ? value.normalize('NFKC').replace(/\p{Cf}/gu, '') : value;
  return typeof normalized === 'string' &&
    (/^--[A-Za-z0-9_-]*(?:api[_-]?key|token|secret|password|credential)[A-Za-z0-9_-]*(?:=|$)/iu.test(normalized) ||
      /^(?:export\s+)?(?=[A-Za-z_])[A-Za-z0-9_-]*(?:api[_-]?key|token|secret|password|credential)[A-Za-z0-9_-]*=/iu.test(normalized));
}

function safeCommandArgument(value) {
  const normalized = typeof value === 'string' ? value.normalize('NFKC').replace(/\p{Cf}/gu, '') : value;
  if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') < 1 || Buffer.byteLength(value, 'utf8') > 240 ||
      typeof normalized !== 'string' || /[\0\r\n`$;&|><]/u.test(normalized) || hasCredentialCommandArgument(normalized)) return false;
...
function hasCopyableSourceSyntax(value) {
  return /(?:^|[^\w$])(?:async\s+)?function\s+[A-Za-z_$][A-Za-z0-9_$]*\s*\([^()\n]{0,400}\)\s*(?::\s*[^\n{]{1,160})?\s*\{/mu.test(value) ||
    /(?:^|[^\w$])(?:const|let|var)\s+[A-Za-z_$][A-Za-z0-9_$]*\s*=\s*(?:async\s*)?\([^()\n]{0,400}\)\s*(?::\s*[^\n=]{1,160})?\s*=>/mu.test(value) ||
    /(?:^|[^\w$])(?:async\s+)?def\s+[A-Za-z_][A-Za-z0-9_]*\s*\([^()\n]{0,400}\)\s*(?:->\s*[^:\n]{1,160})?:/mu.test(value) ||
    /(?:^|[^\w$])func\s+(?:\([^\n)]{1,160}\)\s*)?[A-Za-z_][A-Za-z0-9_]*\s*\([^()\n]{0,400}\)[^{\n]{0,160}\{/mu.test(value) ||
    /(?:^|[^\w$])(?:export\s+)?(?:class|interface|enum)\s+[A-Za-z_$][A-Za-z0-9_$]*[^\n{]{0,240}\{/mu.test(value) ||
...
function isHumanReadablePathComponent(value) {
  const component = value.toLocaleLowerCase('en-US');
  return HUMAN_READABLE_PATH_COMPONENTS.has(component) || HUMAN_READABLE_PATH_EXTENSIONS.has(component) ||
    /^v?\d{1,4}$/u.test(component);
}

function hasHighEntropyAcrossPathComponents(value) {
  const opaqueComponents = [];
  for (const segment of value.split('/')) {
    for (const component of segment.split(/[._-]+/u)) {
      if (component && !isHumanReadablePathComponent(component)) opaqueComponents.push(component);
    }
  }
  return hasHighEntropyWindow(opaqueComponents.join(''));
}

function isLowRiskRelativePathToken(value) {
  if (!safeRelativePath(value) || !isRelativePathToken(value)) return false;
  // Path punctuation is only a presentation boundary. Remove recognized
  // human-readable components, then scan every remaining fragment together
  // so `-`, `_`, `.`, and `/` cannot split an opaque credential into short
  // tokens while ordinary names such as platform/documentation stay useful.
  return !hasHighEntropyAcrossPathComponents(value);
}

function hasHighEntropyCredential(value, ignoredTokens, { allowRelativePathToken = false } = {}) {
  for (const match of value.matchAll(/[A-Za-z0-9+/_=-]{20,256}/gu)) {
    ...
    if (allowRelativePathToken && isRelativePathToken(token)) {
      // Do not let a low-entropy suffix dilute a credential-bearing segment.
      // Once a path-shaped token fails the segment gate, reject it directly.
      if (isLowRiskRelativePathToken(token)) continue;
      return true;
    }
...
function hasHighEntropyAfterDecimalCodeCollapse(value) {
  for (const match of value.matchAll(/(?:[A-Za-z]\d{3}){20,}/gu)) {
...
function hasUnsafeUnicodeFormatOrControl(value) {
  return /\p{Cf}/u.test(value) || /[\u0000-\u0009\u000B-\u001F\u007F-\u009F]/u.test(value);
}

function safetyScanVariants(value) {
  const normalized = value.normalize('NFKC');
  const variants = normalized === value ? [value] : [value, normalized];
  const withoutUnsafeFormatOrControl = normalized
    .replace(/\p{Cf}/gu, '')
    .replace(/[\u0000-\u0009\u000B-\u001F\u007F-\u009F]/gu, '');
  if (withoutUnsafeFormatOrControl !== normalized) variants.push(withoutUnsafeFormatOrControl);
  return variants;
}
...
function inspectSubmissionTextWithOptions(value, options = {}) {
  ...
  for (const candidate of safetyScanVariants(value)) {
    if (/-----BEGIN (?:[A-Z ]+)?PRIVATE KEY-----/iu.test(candidate)) return { ok: false, category: 'private_key' };
    if (/\bauthorization\s*:/iu.test(candidate) || /\bbearer\s+[A-Za-z0-9._~+/=-]{8,}/iu.test(candidate)) {
      return { ok: false, category: 'authorization' };
    }
    if (hasCredentialAssignment(candidate) || hasCredentialCommandArgument(candidate) ||
        /\b(?:sk-(?:proj-)?|gh[pousr]_|glpat-|xox[baprs]-)[A-Za-z0-9_-]{12,}\b/iu.test(candidate) ||
        /\bAKIA[0-9A-Z]{16}\b/u.test(candidate)) {
      return { ok: false, category: 'credential_pattern' };
    }
    if (hasEnvironmentAssignment(candidate)) return { ok: false, category: 'env_assignment' };
    if (!serialized && containsRawMarkdown(candidate)) {
      return { ok: false, category: 'raw_markup' };
    }
    if (!serialized && hasCopyableSourceSyntax(candidate)) return { ok: false, category: 'source_syntax' };
    if (hasExternalUrl(candidate)) return { ok: false, category: 'external_url' };
    if (hasPatchShape(candidate) || /Traceback \(/u.test(candidate) ||
        /\bat\s+(?:async\s+)?[A-Za-z0-9_$.]+\s+\([^()\n]+:\d+(?::\d+)?\)/u.test(candidate)) {
      return { ok: false, category: 'log_or_diff' };
    }
    if ((candidate.match(/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.\d+)?(?:Z|[+-]\d\d:\d\d)\b.*$/gmu) ?? []).length >= 2 ||
        (candidate.match(/^[A-Za-z_][A-Za-z0-9_]{2,}\s*=.*$/gmu) ?? []).length >= 3) {
      return { ok: false, category: 'log_or_env_assignment' };
    }
    if (/["“][^"”\n]{120,}["”]/u.test(candidate)) return { ok: false, category: 'long_verbatim_quote' };
    if (options?.skipHighEntropyCredential !== true && hasHighEntropyCredential(candidate, options?.ignoredHighEntropyTokens, {
      allowRelativePathToken: options?.allowRelativePathToken === true
    })) {
      return { ok: false, category: 'high_entropy_credential' };
    }
  }
  if (hasUnsafeUnicodeFormatOrControl(value)) return { ok: false, category: 'unicode_format_or_control' };
  return { ok: true };
}
...
function collectSafetyTexts(document) {
  ...
  document.sections.forEach((section, sectionIndex) => {
    ...
    section.files.forEach((file, fileIndex) => {
      texts.push({
        path: `$.sections[${sectionIndex}].files[${fileIndex}].path`,
        value: file.path,
        allowRelativePathToken: true
      });
    });
    section.implementation_specs.forEach((specification, specificationIndex) => {
      texts.push({
    ...
    section.commands.forEach((command, commandIndex) => {
      command.args.forEach((value, index) => {
        texts.push({
          path: `$.sections[${sectionIndex}].commands[${commandIndex}].args[${index}]`,
          value,
          allowRelativePathToken: true
        });
      });
...
export async function validateAndSerializeSafeDocument(content, projectRoot, {
  ...
  for (const text of safetyTexts) {
    const inspected = inspectSubmissionTextWithOptions(text.value, {
      allowRelativePathToken: text.allowRelativePathToken === true
    });
  ...
  const inspected = inspectSubmissionTextWithOptions(markdown, {
    serialized: true,
    ignoredHighEntropyTokens,
    // Keep compatibility for ordinary structured paths, but re-run the same
    // segment-level entropy gate so serialization cannot become a bypass.
    allowRelativePathToken: true
  });
