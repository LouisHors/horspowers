...
function isSafeAbsolutePath(value) {
  return isAbsolutePath(value) && !/[\0\r\n;&|$`<>]/u.test(value) && !/:\/\//u.test(value) &&
    !value.startsWith('//') && !value.split(path.sep).includes('..');
}
...
export function validateContextInput(input) {
  ...
  if (input.wiki_root !== null && !isSafeAbsolutePath(input.wiki_root)) throw new TypeError('wiki_root must be null or a safe absolute path');
  ...
  for (const entry of input.known_entry_files) {
    if (!isSafeAbsolutePath(entry) || !insideRoot(entry, input.cwd)) throw new TypeError('known entry files must be safe absolute paths inside cwd');
  }
  return input;
}
...
function trustedWikiRootFromRuntime(runtimeResult) {
  const provenance = runtimeResult?.wiki_root_provenance;
  if (runtimeResult?.status !== 'ready' || runtimeResult?.identity_status !== 'external' ||
      runtimeResult?.wiki_root_trusted !== true || typeof runtimeResult?.wiki_root !== 'string' ||
      provenance?.source !== 'validated_host_config' || provenance?.field !== 'wiki.local_root' ||
      provenance?.canonical !== true) return null;
  // The request-side root is untrusted input. Once the DocumentRuntime has
  // validated the canonical Wiki root, always use that runtime-provided path
  // regardless of what the caller supplied (or omitted).
  return runtimeResult.wiki_root;
}
...
async function defaultResolveRuntime(cwd) {
  try {
    const { DocumentRuntime } = await import('./document-runtime.mjs');
...
// `git ls-files` C-quotes paths that contain non-ASCII or special bytes unless
// `-z` is used. Split on NUL when present so a quoted filename can never turn
// into a bogus grep argument; keep newline splitting for the rg/find
// enumerators and injected test doubles.
function splitFileList(stdout) {
...
async function enumerateFiles({ capabilities, runCommand, cwd, signal }) {
  ...
  if (capabilities.git) return { tool: 'git ls-files', result: await runSearch(runCommand, 'git', ['ls-files', '-z'], cwd, 3_000, signal) };
...
async function collectRepository(context, dependencies, signal) {
  ...
  try {
    ...
    if (capabilities.rg) {
      ...
      const candidates = splitFileList(listed.result.stdout).filter((file) => file && !isSensitivePath(file)).slice(0, 200);
      tool = 'git grep -n';
      ...
      const listed = await enumerateFiles({ capabilities, runCommand, cwd: context.cwd, signal });
      const candidates = splitFileList(listed.result.stdout).filter((file) => file && !isSensitivePath(file)).slice(0, 200).map((file) => path.isAbsolute(file) ? file : path.join(context.cwd, file));
...
async function collectWiki(context, dependencies, signal) {
  ...
  try {
    ...
    // A caller-supplied absolute path is not proof that it is the user's Wiki.
    // Only a runtime-resolved root, echoed with an explicit trust marker, may
    // reach the local grep fallback. This keeps qmd unavailable fail-closed
    // instead of turning context collection into arbitrary path traversal.
    const trustedRoot = trustedWikiRootFromRuntime(runtimeResult);
    if (!trustedRoot) return branch({ status: 'skipped', errorCode: 'NO_TRUSTED_WIKI', durationMs: performance.now() - started });
    const result = await runSearch(runCommand, 'grep', ['-RIn', ...grepExclusions(), '--include=*.md', '--', context.query, trustedRoot], trustedRoot, 4_000, signal);
    ...
    const items = parseTextMatches(result.stdout, trustedRoot, 'wiki', 8);
...
async function collectEntries(context, dependencies, signal) {
  ...
  try {
    const canonicalRoot = await dependencies.realpath(context.cwd);
    const reads = await Promise.all(entries.map(async (entry) => {
      ...
      try {
        const canonicalEntry = await dependencies.realpath(entry);
        if (!insideRoot(canonicalEntry, canonicalRoot) || isSensitivePath(canonicalEntry)) return { sensitive: true };
        const content = await dependencies.readFile(canonicalEntry, MAX_ENTRY_BYTES);
        // Re-resolve the caller-visible path after the read. This cannot make
        // arbitrary pathname access atomic on every supported OS, but it
        // detects the common symlink-swap race while the default reader also
        // refuses to follow a final symlink.
        const verifiedEntry = await dependencies.realpath(entry);
        if (verifiedEntry !== canonicalEntry || !insideRoot(verifiedEntry, canonicalRoot)) return { sensitive: true };
...
function closeLateRuntime(value) {
  const qmdClient = value?.wiki?.qmd_client ?? value?.qmd_client;
  if (!qmdClient || typeof qmdClient.close !== 'function') return;
  return Promise.resolve().then(() => qmdClient.close()).catch(() => {});
}

function waitForDeadline(factory, signal, { onLateValue } = {}) {
  return new Promise((resolve) => {
    let settled = false;
    const finish = (value) => {
      if (settled) return;
    ...
    Promise.resolve().then(() => factory(signal)).then(
      (value) => {
        if (settled) {
          // A resolver may allocate a resource after the deadline has already
          // won the race. Give the caller one chance to release that late
          // value instead of leaking a qmd transport in the background.
          Promise.resolve().then(() => onLateValue?.(value)).catch(() => {});
          return;
        }
...
export async function collectContext(input, overrides = {}) {
  ...
  try {
    const resolved = await waitForDeadline(
      (signal) => resolveRuntime(context.cwd, signal),
      controller.signal,
      { onLateValue: closeLateRuntime }
    );
    ...
    if (!capabilities) {
      const detected = await waitForDeadline(
        (signal) => detectCapabilities(context.cwd, runtimeResult?.identity_status === 'external', signal),
        controller.signal
      );
    ...
    const dependencies = {
      ...
      realpath: overrides.realpath ?? realpathFromDisk,
      runtimeResult
    };
    const states = {};
