 ...
 */
function buildConfigForMode(mode = 'personal') {
  return {
    ...DEFAULT_CONFIG,
...
/**
 * 使用与既有写入 API 相同的格式序列化配置。
 * @param {Object} config - 配置对象
 * @param {string} generatedAt - 生成时间
 * @returns {string} YAML 文本
 */
function serializeConfig(config, generatedAt = new Date().toISOString()) {
  return `# Horspowers Configuration
...
function readConfigAtRoot(projectDir) {
...
/**
 * 描述项目根目录已有配置的状态，不读取父目录配置。
 * @param {string} projectDir - 项目根目录
 * @returns {'missing'|'valid'|'invalid'|'needs_update'|'needs_migration'}
 */
function classifyConfigAtRoot(projectDir) {
  const files = detectConfigFiles(projectDir);
  if (files.hasNew) {
    const config = readConfigAtRoot(projectDir);
    if (!config || !validateConfig(config).valid) return 'invalid';
    return checkConfigUpdate(config).needsUpdate ? 'needs_update' : 'valid';
  }
  if (files.hasOld) return 'needs_migration';
  return 'missing';
}
...
/**
 * 仅在配置缺失时原子创建，绝不覆盖已有配置。
 * @param {string} projectDir - 项目根目录
 * @param {string} mode - 开发模式 ('personal' | 'team')
 * @returns {Promise<Object>} 创建或现有配置状态
 */
async function initializeConfigIfMissing(projectDir, mode = 'team') {
  const files = detectConfigFiles(projectDir);
  if (files.hasOld && !files.hasNew) {
    return { status: 'exists', config_state: 'needs_migration' };
  }
  ...
  try {
    ...
    if (error && error.code === 'EEXIST') {
      // A competing `wx` writer may have created the file but not finished writing it.
      // Retry only the read classification; never rewrite or replace its content.
      let configState = 'invalid';
      for (let attempt = 0; attempt < 5; attempt += 1) {
        configState = classifyConfigAtRoot(projectDir);
        if (configState !== 'invalid') break;
        await new Promise((resolve) => setTimeout(resolve, 5));
      }
      return { status: 'exists', config_state: configState };
    }
