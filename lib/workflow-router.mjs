...
import { EXTERNAL_DOCUMENT_RUNTIME_VERSION } from './document-runtime-capabilities.mjs';
...
const defaultDependencies = {
  ...
  resolveProjectContext: ({ cwd, projectPlan }) => resolveProjectContext({
    cwd,
    homeDir: homedir(),
    identity: projectPlan?.identity,
    projectRoot: projectPlan?.project_root
  }),
...
function contextFields(projectPlan, projectContext) {
  const isExternal = projectPlan?.eligibility === 'external_project';
  return {
    identity_status: projectContext?.project?.identity_status ?? projectPlan?.identity?.kind ?? null,
    project_id: projectContext?.project?.project_id ?? null,
    project_fingerprint: projectContext?.project?.project_fingerprint ?? projectPlan?.identity?.project_fingerprint ?? null,
    config_source: projectContext?.config?.source ?? (isExternal ? 'none' : null),
    documentation_backend: projectContext?.documentation?.backend ?? (isExternal ? 'disabled' : null),
    documentation_status: projectContext?.status ?? (isExternal ? 'context_unavailable' : null),
    auto_submit: projectContext?.documentation?.auto_submit === true
  };
}

function unavailableExternalContext(projectPlan) {
  return {
    status: 'context_unavailable',
    project: {
      identity_status: projectPlan?.identity?.kind ?? null,
      project_id: null,
      project_fingerprint: projectPlan?.identity?.project_fingerprint ?? null
    },
    config: { source: 'none', value: null },
    documentation: { backend: 'disabled', enabled: false, auto_submit: false }
  };
}

function buildStableOutput({ routing, agents, project, projectPlan, projectContext }) {
  ...
  return {
    ...
    project: {
      ...
      docs: docs.status,
      ...contextFields(projectPlan, projectContext)
    },
    routing,
    mutations: [
...
function applyExternalRuntimeSafetyGate(routing, projectPlan, externalDocumentRuntimeVersion) {
  if (projectPlan.eligibility !== 'external_project' || externalDocumentRuntimeVersion >= 1) {
    return routing;
  }
  return {
    ...routing,
    target_skill: null,
    blocked_by: 'company_external_config_required'
  };
}
...
export async function routeRequest(input, dependencies = {}) {
  ...
  const externalDocumentRuntimeVersion = Number.isInteger(dependencies.externalDocumentRuntimeVersion) &&
    dependencies.externalDocumentRuntimeVersion >= 0
    ? dependencies.externalDocumentRuntimeVersion
    : EXTERNAL_DOCUMENT_RUNTIME_VERSION;
  let routing;
  try {
  ...
  routing = applyExternalRuntimeSafetyGate(routing, projectPlan, externalDocumentRuntimeVersion);
  let projectContext = null;
  if (projectPlan.eligibility === 'external_project') {
    try {
      projectContext = await resolvedDependencies.resolveProjectContext({
        cwd: input.cwd,
        projectPlan
      });
    } catch {
      projectContext = unavailableExternalContext(projectPlan);
    }
  }
  ...
  return buildStableOutput({ routing, agents, project, projectPlan, projectContext });
}
