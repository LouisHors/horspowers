...
export async function routeRequest(input, { rulesPath = process.env.HORSPOWERS_ROUTE_RULES_PATH } = {}) {
  validateInput(input);
  let routing;
  try {
    const rules = await loadAndValidateRules(rulesPath || undefined);
    routing = classifyRequest(input, rules);
  } catch (error) {
    routing = fallbackUncertain(error);
  }
