...
/**
 * 生成零写入的项目初始化方案。
 * @param {{cwd: string, homeDir: string, tempDir: string, localProjectRoots?: Array<string>, platform?: string}} options
 * @returns {Promise<Object>}
 */
export async function planProjectInitialization({ cwd, homeDir, tempDir, localProjectRoots, platform }) {
  ...
  if (gitRoot) {
    ...
    const identity = await identifyGitProject(gitRoot, {
      ...(Array.isArray(localProjectRoots) ? { localProjectRoots } : {}),
