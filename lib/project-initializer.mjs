...
async function findMarkedProjectRoot(cwd) {
  let current = cwd;
  while (true) {
...
export async function planProjectInitialization({ cwd, homeDir, tempDir }) {
  ...
  const markerRoot = await findMarkedProjectRoot(resolvedCwd);
  const gitRoot = markerRoot ? null : await getGitRoot(resolvedCwd);
