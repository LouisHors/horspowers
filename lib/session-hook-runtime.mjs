...
function hpsModulePaths() {
  return [
    path.join(pluginRoot, 'lib', 'hps-runtime.mjs'),
    path.join(pluginRoot, 'lib', 'hps-operations.mjs')
  ];
}
...
function runtimeCliPath() {
  // Hooks always execute the runtime shipped with their own installation.
  // Tests use an isolated fixture installation instead of an environment override.
...
async function sessionStart() {
  ...
  const [skill, warning, upgrade, hpsStatus] = await Promise.all([
    skillContent(),
    legacyWarning(),
  ...
  const context = [
    ...
    `<hps-runtime status="${hpsStatus}">Shared HPS Core boundary checked without project mutation.</hps-runtime>`,
    'On the next substantive user message, use the HPS installation root discovered by the host and call `<installation-root>/bin/hps call` (or the registered `hps serve --stdio` MCP) through safe stdin. Only when HPS is unavailable may the documented route-request.mjs compatibility wrapper run once. SessionStart performs no project initialization, qmd query, or target-Skill routing.',
    upgrade,
    warning,
    '</EXTREMELY_IMPORTANT>'
  ].filter(Boolean).join('\n\n');
...
/**
 * Establish a short-lived shared-Core scope and record only bounded document
 * references in HPS. The document runtime remains the compatibility writer
 * for actual archive/Inbox effects; HPS owns the deterministic session gate
 * and idempotency boundary. Missing HPS modules are treated as an explicit
 * compatibility case so older installations keep their hook protocol.
 */
async function hpsSessionGate(refs) {
  if (!await Promise.all(hpsModulePaths().map(target => exists(target))).then(results => results.every(Boolean))) return null;
  let createHpsRuntime;
  let dispatchHpsOperation;
  try {
    ({ createHpsRuntime } = await import('./hps-runtime.mjs'));
    ({ dispatchHpsOperation } = await import('./hps-operations.mjs'));
  } catch {
    return null;
  }
  ...
  timer.unref?.();
  try {
    const host = process.env.HORSPOWERS_HOST === 'codex' ? 'codex' : 'claude';
    const prepared = await dispatchHpsOperation({
      runtime,
      operation: 'task_prepare',
      cwd: process.cwd(),
      input: { message: '', active_route: null, host },
      signal: controller.signal
    });
    const scopeId = prepared?.scope?.scope_id;
    if (typeof scopeId !== 'string' || scopeId.length === 0) return { handled: false };
    const project = prepared.project ?? {};
    const context = prepared.context ?? null;
    if (project.eligibility === 'external_project' && context?.status !== 'ready') {
      return { handled: true, persisted: false, status: documentationState(context) };
    }
    await dispatchHpsOperation({
      ...
      input: {
        scope_id: scopeId,
        request_id: boundedOpaqueSessionId(process.env.CLAUDE_SESSION_ID),
        idempotency_key: `hook-${boundedOpaqueSessionId(process.env.CLAUDE_SESSION_ID)}`,
        references: refs.map(reference => ({ logical_id: reference.logical_id, status: null, revision: null }))
      },
      signal: controller.signal
    });
...
async function hpsDoctorStatus() {
  if (!await Promise.all(hpsModulePaths().map(target => exists(target))).then(results => results.every(Boolean))) {
    return 'compatibility';
  }
  try {
    const [{ createHpsRuntime }, { dispatchHpsOperation }] = await Promise.all([
      import('./hps-runtime.mjs'), import('./hps-operations.mjs')
    ]);
    const runtime = createHpsRuntime();
    try {
      const result = await dispatchHpsOperation({
        runtime, operation: 'runtime_doctor', cwd: process.cwd(), input: {}
      });
      return result ? 'ready' : 'unavailable';
    } finally {
      await runtime.close?.();
    }
  } catch {
...
async function sessionEnd() {
  ...
  const refs = supplied.supplied
    ...
    : resolution.backend === 'local'
      ...
      : [];
  const hpsGate = await hpsSessionGate(refs);
  if (hpsGate?.handled && hpsGate.persisted === false) {
    emit('SessionEnd', `${hpsGate.status ?? 'runtime-unavailable'}; documentation not persisted`);
    return;
  }
