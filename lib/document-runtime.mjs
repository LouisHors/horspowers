...
import { inspectSubmissionMetadataIdentifier } from './submission-safety.mjs';
...
function publicProjectId(value) {
  return typeof value === 'string' && inspectSubmissionMetadataIdentifier(value, { projectId: true })?.ok === true
    ? value
    : null;
}
...
function resolution(status, backend, projectId, errorCode = null, context = null) {
  return {
    status,
    backend,
    project_id: publicProjectId(projectId),
    ...(errorCode ? { error_code: errorCode } : {}),
    ...resolutionMetadata(context)
  };
}

function outcomeProjectId(outcome, fallback) {
  const projectId = outcome !== null && typeof outcome === 'object' && Object.hasOwn(outcome, 'project_id')
    ? outcome.project_id
    : fallback;
  return publicProjectId(projectId);
}
...
export class DocumentRuntime {
  ...
  async select(cwd, { action = null } = {}) {
    ...
    const canChangeDisabledWikiConfig = action === 'config-change' &&
      documentation?.enabled === false &&
      documentation.backend === 'disabled' &&
      context.config?.source === 'wiki' &&
      context.config?.value?.documentation?.backend === 'wiki';
    if ((!documentation || documentation.enabled !== true || documentation.backend === 'disabled') &&
        !canChangeDisabledWikiConfig) {
      return {
        result: resolution('documentation_disabled', 'disabled', projectId, 'documentation_disabled', context),
        context
      };
    }
    if (canChangeDisabledWikiConfig) {
  ...
  async execute({ cwd, action, request = {}, confirmed = false } = {}) {
    ...
    const selected = await this.select(cwd, { action });
    ...
    if (selected.result.backend === 'local') {
      ...
      try {
        ...
        return {
          ...
          project_id: outcomeProjectId(outcome, selected.result.project_id)
        };
      } catch {
