...
export async function resolveWikiProjectConfig({ identity, hostConfig, qmdClient } = {}) {
  ...
  if (!extractedRegistry.ok) return { status: 'registry_invalid', error_code: extractedRegistry.error_code };
  ...
  if (!extractedConfig.ok) return { status: 'project_config_invalid', error_code: extractedConfig.error_code };
  ...
  if (!extractedManifest.ok) return { status: 'project_config_invalid', error_code: extractedManifest.error_code };
