...
export class WikiDocsBackend {
  ...
  #inspectProjectConfig(value) {
    // `project_id` is bounded but intentionally opaque. Inspect both the
    // complete identifier and each slash-separated segment so a token cannot
    // hide inside an otherwise path-like project ID.
    const projectIdInspection = this.inspectText(value.project_id);
    if (!projectIdInspection?.ok) return projectIdInspection;
    for (const segment of value.project_id.split('/')) {
      const segmentInspection = this.inspectText(segment);
      if (!segmentInspection?.ok) return segmentInspection;
    }
    const scanValue = {
      ...value,
