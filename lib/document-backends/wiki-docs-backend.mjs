...
function safeIssues(errors) {
  ...
  const sanitized = errors.slice(0, 32).flatMap((error) => {
    ...
    return [{
      ...
      code: SAFE_ISSUE_CODES.has(error.code) ? error.code : 'validation_failed'
    }];
  });
...
export class WikiDocsBackend {
  constructor(options = {}, dependencies = {}) {
    ...
    this.inspectText = dependencies.inspectSubmissionText ?? options.inspectSubmissionText ?? inspectSubmissionText;
  ...
  #sameWikiLocation(candidate) {
    ...
    return actual?.collection === expected.collection && actual?.root_uri === expected.root_uri &&
      ...
      actual?.enabled === true && actual?.submission?.mode === 'inbox-only';
  }
  ...
  async #prepareMutation(mutation) {
    ...
    if (mutation.operation === 'create' || mutation.operation === 'update') {
      return this.#prepareDocumentMutation(mutation, loaded.manifest);
    }
    if (mutation.operation === 'config-change') {
      return this.#prepareConfigMutation(mutation, loaded.manifest, {
        entry: loaded.configEntry,
        config: loaded.currentConfig
      });
    }
    return this.#prepareTransitionMutation(mutation, loaded.manifest);
  }

  #canSubmit(confirmed) {
    return this.config?.documentation?.submission?.auto_submit === true || confirmed === true;
  }
  ...
  async mutateBatch(mutations, { confirmed = false } = {}) {
    ...
    const prepared = [];
    for (const suppliedMutation of mutations) {
      const mutation = isPlainObject(suppliedMutation) &&
        hasOnlyKeys(suppliedMutation, ['operation', 'mutation'], ['operation', 'mutation'])
        ? { operation: suppliedMutation.operation, ...suppliedMutation.mutation }
        : suppliedMutation;
      const outcome = await this.#prepareMutation(mutation);
      if (!outcome.ok) return outcome.result;
      prepared.push(outcome.prepared);
    }
    if (!this.#canSubmit(confirmed)) {
      return this.#result('confirmation_required', { previews: prepared.map(previewFor) });
    }

    const submitted = [];
    const failures = [];
    for (const entry of prepared) {
      const outcome = await this.#submitPrepared(entry);
  ...
  #recordSessionRequest(request) {
    ...
    for (const reference of refs) {
      if (!hasOnlyKeys(reference, ['document_type', 'logical_id'], ['document_type', 'logical_id']) ||
          !DOCUMENT_TYPES.has(reference.document_type) || !validLogicalId(reference.logical_id)) {
        return { ok: false, result: this.#failure('invalid_request', 'session_refs_invalid') };
      }
  ...
  async recordSession(request = {}, options = {}) {
    ...
    const sessionDigest = sha256(request.session.session_id).slice(0, 32);
    const sessionLogicalId = `session-${sessionDigest}`;
    const referenceBullets = validated.refs.map((reference) => {
      const entry = loaded.manifest.documents[reference.logical_id];
      return `Reference ${reference.document_type}/${reference.logical_id}: ${entry.status} (revision ${entry.revision}).`;
    });
