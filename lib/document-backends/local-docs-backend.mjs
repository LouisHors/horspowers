...
const COMPLETED_DOCUMENT_STATUSES = new Set(['已完成', '已修复', '已关闭', 'completed', 'fixed']);
const LEGACY_COMPLETED_DOCUMENT_STATUS = /(?:^|\n)[ \t]*-[ \t]*(?:状态|status)[：:][ \t]*(?:已完成|已修复|已关闭|completed|fixed)[ \t]*(?:\r?\n|$)/iu;
...
function isCompletedDocument(content) {
  ...
  if (status !== null) return COMPLETED_DOCUMENT_STATUSES.has(status);

  // SessionEnd historically recognized a completed marker anywhere in an
  // older active Markdown file. Preserve that fallback only when the document
  // has no current-status section, so a historical progress entry cannot
  // override an explicit active status in `## 基本信息`.
  if (typeof content !== 'string' || /^## 基本信息[ \t]*\r?$/mu.test(content)) return false;
  return LEGACY_COMPLETED_DOCUMENT_STATUS.test(content);
}
...
export class LocalDocsBackend {
  ...
  /**
   * Keep the legacy SessionEnd behavior inside the local backend: every
   * completed Markdown document in the trusted active directory is eligible
   * for archiving, even when the hook supplied no explicit task/bug reference.
   * Explicit references remain the only documents that receive session
   * progress updates.
   */
  resolveCompletedActiveDocuments() {
    if (typeof this.manager?.getTrustedDocumentDirectory !== 'function' ||
        typeof this.manager?.getDocument !== 'function' || typeof this.manager.activeDir !== 'string') {
      return {
        ok: false,
        result: this.result('operation_failed', { error_code: 'session_archive_resolution_failed' })
      };
    }

    const activeDirectory = this.manager.getTrustedDocumentDirectory(this.manager.activeDir);
    if (!activeDirectory?.success) {
      return { ok: false, result: this.failure(activeDirectory, 'session_archive_resolution_failed') };
    }
    ...
    for (const entry of entries) {
      ...
      const documentPath = path.resolve(activeDirectory.path, entry.name);
      if (!isPathWithin(activeDirectory.path, documentPath)) {
        return {
          ok: false,
          result: this.result('invalid_request', { error_code: 'session_archive_not_active' })
        };
      }
      const document = this.manager.getDocument(documentPath);
      if (!document?.success) return { ok: false, result: this.failure(document, 'session_archive_resolution_failed') };
      if (!isPathWithin(activeDirectory.path, document.path)) {
        return {
          ok: false,
          result: this.result('invalid_request', { error_code: 'session_archive_not_active' })
        };
      }
      if (isCompletedDocument(document.content)) {
        documents.push({ path: document.path, content: document.content, completed: true });
      }
    }
  ...
  async recordSession(request = {}) {
    ...
    const completed = request.auto_archive_completed === true
      ? this.resolveCompletedActiveDocuments()
      : { ok: true, documents: [] };
    ...
    const knownPaths = new Set(changes.map(change => change.reference.path));
    for (const document of completed.documents) {
      if (knownPaths.has(document.path)) continue;
      knownPaths.add(document.path);
      changes.push({ reference: document, updateAttempted: false, archivedPath: null });
    }
