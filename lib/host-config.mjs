...
function validateObject(value, pathname, allowedKeys, errors, requiredKeys = allowedKeys) {
...
function normalizeConfig(value) {
  return {
    ...
    wiki: {
      ...
      ...(value.wiki.local_root === undefined ? {} : { local_root: value.wiki.local_root }),
      inbox: {
...
export function validateHostConfig(value) {
  ...
  const wiki = validateObject(
    root.wiki,
    '$.wiki',
    ['transport', 'collection', 'registry_uri', 'inbox', 'local_root'],
    errors,
  ...
  if (wiki) {
    ...
    if (wiki.local_root !== undefined && (
      typeof wiki.local_root !== 'string' || !path.isAbsolute(wiki.local_root) ||
      /[\0\r\n]/u.test(wiki.local_root) || wiki.local_root.startsWith('//') ||
      wiki.local_root.split(path.sep).includes('..') || /:\/\//u.test(wiki.local_root)
    )) {
      errors.push(error('$.wiki.local_root', 'safe_absolute_path_required'));
    }
  }
