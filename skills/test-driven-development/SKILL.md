...
当用户明确要求先写 failing case、失败测试或复现测试时，本 Skill 必须直接接管首个动作；即使具体功能或缺陷尚未给出，也不能把该请求改成泛化澄清流程。

- 首次回复必须明确说出“测试驱动开发”或 `test-driven-development`，并说明下一步先固定一个失败或复现测试。
- 具体目标尚未给出时，最多追问一个简短问题；这不能取代上述声明。
- 在首次回复前不得检查仓库、运行测试、给出实现方案或写生产代码。
